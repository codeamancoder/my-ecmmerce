...
class Arama extends Public_Controller
{
	...
	//var_export($_POST); return;
		...
		//var_export($manufacturers); return;
		// PAGINATION
		...
		$uri				= site_url('urun/arama/index?' . $write);
		$uri_segment 		= NULL;
		$total_rows			= $result['toplam'];
		if($result) {
			$pagination 	= create_pagination($uri, $total_rows, $limit, $uri_segment, 'site', TRUE);
		} else {
			$pagination 	= FALSE;
		}
		...
		$this->template->set_master_template(tema() . 'urun/arama_sonuclari');
		...
		$this->template->write_view('content', tema() . 'urun/arama_sonuclari_content',$data);
