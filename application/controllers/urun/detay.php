...
class detay extends Public_Controller {
	...
	function index($product_seo)
	{
		...
		switch ( config('site_ayar_urundetay_urungrubu') ) {
			case 'benzer_urunler':
				$content_data["related_type"] = "benzer_urunler";
				$content_data['product_related'] = $this->product_model->get_related_by_id($product_info->product_id);
				$content_data['related_title'] = lang('messages_product_detail_related_products');
				break;
				
			case 'daha_ucuz_urunler':
				$content_data["related_type"] = "daha_ucuz_urunler";
				$content_data['product_related'] = $this->product_model->get_related_cheap_products($product_info->product_id);
				$content_data['related_title'] = lang('messages_product_detail_cheap_products');
				break;
				
			case 'kampanyali_urunler':
				$content_data["related_type"] = "kampanyali_urunler";
				$content_data['product_related'] = $this->product_model->get_related_campaign_product($product_info->product_id);
				$content_data['related_title'] = lang('messages_product_detail_campaign_products');
				break;
				
			case 'indirimli_urunler':
				$content_data["related_type"] = "indirimli_urunler";
				$content_data['product_related'] = $this->product_model->get_related_discount_product($product_info->product_id);
				$content_data['related_title'] = lang('messages_product_detail_discount_products');
				break;
				
			default:
				$content_data['product_related'] = false;
				break;
		...
		$this->template->set_master_template(tema() . 'urun/index');

		$this->template->add_region('content');
		$this->template->write_view('content', tema() . 'urun/content', $content_data);
		$this->template->add_css(APPPATH . 'views/' . tema_asset() . 'css/urundetay.css');
		$this->template->add_css(APPPATH . 'views/' . tema_asset() . 'css/jqzoom.css');
		$this->template->add_css(APPPATH . 'views/' . tema_asset() . 'css/jquery.lightbox-0.5.css');
		$this->template->add_js(APPPATH . 'views/' . tema() . 'js/jqzoom.pack.1.0.1.js');
