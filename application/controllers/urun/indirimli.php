...
class indirimli extends Public_Controller
{
	...
	function index($sort_link = 'price-asc', $page = 0)
	{
		...
		$this->template->write('baslik', lang('messages_discount_products_title'));
		...
		$uri_segment = 6;
		$new_products = $this->product_model->get_discount_product($sort, $order, $page, $limit);

		$content_data['urunler'] = $new_products;
		if($new_products) {
			$content_data['pagination'] = create_pagination(site_url('urun/indirimli/index/' . $sort_link), $new_products['total'], $limit, $uri_segment);
		} else {
			$content_data['pagination'] = FALSE;
		}

		$this->template->set_master_template(tema() . 'urun/indirimli');
		$this->template->add_region('content');
		$this->template->write_view('content', tema() . 'urun/indirimli_content',$content_data);
