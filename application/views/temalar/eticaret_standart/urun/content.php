...
<script type="text/javascript" charset="utf-8">
	$(document).ready(function(){
		...
		$("#sosyal_paylasimlar").jsocial({highlight: true,
			...
			imagedir: "<?php echo site_resim(); ?>sosyal_paylasimlar/kucuk/", 
			imageextension: "gif", 
			blanktarget: true
		});
...
$q1 = $this->db->get_where('e_product_special',array('product_id'=>$product_info->product_id));
$q2 = $this->db->get_where('e_product_discount',array('product_id'=>$product_info->product_id));
if($q1->num_rows()>0)
{
	//print_r($q1->row()); return;
	$row = $q1->row();
...
<?php
 ...
 if ( config('site_ayar_urun_tarih_goster') == '1'  and $q1->num_rows()>0 ) {

 
 $dt = date('Y-m-d H:i:s',$date_end);
...
elseif ( config('site_ayar_urun_kalansure_goster') == '1'  and $q2->num_rows()>0 ) 
{
	...
	<div id="urun_resim_cont" class="sola">
		...
		<?php
			if($product_info->quantity) {
				echo '<form action="'. ssl_url('sepet/ekle/urun_ekle/index') .'" name="sepet_ekle" id="sepet_ekle" method="post">';
				...
				echo '<input type="hidden" name="basket_image" id="basket_image" value="'.$_r_kucuk .'" />';
				echo '<input type="hidden" name="where" id="where" value="urun_detay" />';
			}
		?>
		</div>
		<div id="urun_detay" class="saga">
			...
			<?php
				...
				//SKOCH fixed.( Ürün detayda kdv dahil göster dediğinde göstermiyodu)
				if(config('site_ayar_kdv_goster')=='0')
				{
					$fiyat_g = format_number($fiyat_bilgi['stok_gercek_fiyat_t']*($fiyat_bilgi['kdv_orani']+1));
			...
			<div class="urun_detay_oge">
				...
				<?php if($product_review_avg) { ?>
					<span class="u_oge sola"><img src="<?php echo site_resim(); ?>stars_<?php echo $product_review_avg; ?>.png" alt="Oy <?php echo $product_review_avg; ?>" /></span>
				<?php } else { ?>
			...
			<div id="urun_detay_satinal_butonlar">
				<?php
					if($this->dx_auth->is_logged_in()) {
						...
						$count_follow = $this->product_model->count_follow_by_id($product_info->product_id, $user_id);
						if($count_follow) {
							echo '<span id="takip_et" class="sola" >
							<a class="buton_takip_cikar sola" href="javascript:;" rel="nofollow" onclick="urun_takip_et(\'takip_et\', 0);"><b class="corpi">'. lang('messages_product_detail_dont_follow') .'</b></a>
							</span>';
						} else {
							echo '<span id="takip_et" class="sola" ><a class="buton_takip_et sola" href="javascript:;" rel="nofollow" onclick="urun_takip_et(\'takip_et\', 1);"><b class="cekk">'. lang('messages_product_detail_follow') .'</b></a></span>';
						}
					} else {
						
						
						echo '<a class="buton_takip_et sola" href="javascript:;" rel="nofollow" onclick="redirect(\''. ssl_url('uye/giris?red=' . current_url()) .'\');"><b class="cekk">'. lang('messages_product_detail_follow') .'</b></a>';
					}
				?>
				<?php if(config('site_ayar_fiyat_goster') == '1' AND $this->dx_auth->is_logged_in()) { ?>
					<?php if($product_info->quantity) { ?>
					<?php if(!$product_option) { ?>
					<a class="buton_hizli_al sola" href="javascript:;" rel="nofollow" onclick="redirect('<?php echo ssl_url('odeme/adim_1/hizli/'. $product_info->model); ?>');"><b class="urhizlial"><?php echo lang('messages_product_detail_buy_now'); ?></b></a>
					<?php } ?>
					<a class="buton_sepete_ekle sola" href="javascript:;" rel="nofollow" onclick="sepete_ekle();"><b class="ursepetat"><?php echo lang('messages_product_detail_add_to_cart'); ?></b></a>
					<?php } else { ?>
				...
				<?php } else { ?>
					...
					<a class="buton_sepete_ekle sola" href="javascript:;" rel="nofollow" onclick="sepete_ekle();"><b class="ursepetat"><?php echo lang('messages_product_detail_add_to_cart'); ?></b></a>
					<?php } else {
						 if(config('site_ayar_stok_durumu_goster')){ ?>
					<div class="buton_stok_yok sola" style="display:inline-block;"><b class="urstokyok"><?php echo lang('messages_product_detail_stock_no_stocks'); ?></b></div>
					<?php } } ?>
					<?php } else { ?>
			...
			<div id="urun_detay_butonlar">
				<a href="<?php echo current_url(); ?>" class="u_favori jqbookmark"><?php echo lang('messages_product_detail_add_to_favorites'); ?></a>
...
?>
<script type="text/javascript">
function urun_takip_et(span_id, durum)
{
	$.ajax({
		type: "POST",
		url: site_url('urun/detay/takip_durum'),
		data: "follow_product_id=<?php echo $product_info->product_id; ?>"
		+ "&follow_user_id=<?php echo $this->dx_auth->get_user_id(); ?>"
		+ "&follow_status=" + durum,
		dataType: 'json',
		beforeSend: function() {
			$('#' + span_id).html('<img src="'+ resim_url +'loader.gif" />');
		},
		success: function(data) {
			if (data.basarisiz) {
				if(durum == 1)
				{
					$('#' + span_id).html('<a class="buton_takip_cikar sola" href="javascript:;" rel="nofollow" onclick="urun_takip_et(\'takip_et\', 0);"><?php echo strtr(lang("messages_product_detail_dont_follow"), array("'" => "\'")); ?></a>');
				} else if (durum == 0) {
					$('#' + span_id).html('<a class="buton_takip_et sola" href="javascript:;" rel="nofollow" onclick="urun_takip_et(\'takip_et\', 1);"><?php echo lang("messages_product_detail_follow"); ?></a>');
				}
				$('#takip_sonuc').show();
				$('#takip_sonuc').html('<span style="color:red;width:310px;margin:10px auto 0;display:block;text-align:center;">' + data.basarisiz + '</span>');
				setTimeout("$('#takip_sonuc').stop(true,true).fadeOut(3000)", 3000);
			}
			...
			if (data.basarili) {
				...
				$('#takip_sonuc').show();
				$('#takip_sonuc').html('<span style="color:green;width:310px;margin:10px auto 0;display:block;text-align:center;">' + data.basarili + '</span>');
				setTimeout("$('#takip_sonuc').stop(true,true).fadeOut(3000)", 3000);
			}
		}
	});
...
<?php } ?>
		...
		<div id="urun_tablar">
			...
			<?php } ?>
			<?php if (!empty($product_info->video)) {?>
		...
		<div id="urun_tab_cont">
			...
			<div class="urun_tab_container" id="tab_yorumlar">
				...
				<div class="urun_yorum">
					<div class="uy_yazan"><?php echo $review->author; ?> | <img src="<?php echo site_resim(); ?>stars_<?php echo $review->rating; ?>.png" alt="<?php echo lang('messages_product_detail_comments_vote') . $review->rating; ?>" />
					</div>
			...
			foreach($banka_sorgu->result() as $bankalar) {
				...
				<div class="u_taksit_container">
					...
					<div class="u_taksit_ic">
						<div class="u_taksit_banka"><img height="35" src="<?php echo site_resim(); ?><?php echo $bankalar->kk_banka_resim; ?>" alt="<?php echo $bankalar->kk_banka_adi; ?>" title="<?php echo $bankalar->kk_banka_adi; ?>" /></div>
						<?php
						$tii = 0;
						$oi = 0;
			...
			$digerurun=array();
				if($product_related) {
					...
					foreach($product_related as $benzer) {
						if(!in_array($benzer->product_id, $digerurun) && !preg_match("/\b".$benzer->seo."\b/i", $_SERVER['REQUEST_URI'])){
						$k = $sii%2;
						if($k == 0) {
							$div_class = ' ud_gri';
						} else {
	...
	new AjaxUpload(button, {
		...
		onComplete: function(file, json) {
			...
			if (json.success) {
				...
				$('div#resim').html('<img src="<?php echo "./upload/download/temp/" ?>'+json.file+'" width="50" height="50" />');
